...
par(mfrow=c(2,3)); ylim = c(200,600)
profile.dmc("a",.1,2,p.vector,data.model,ylim=ylim)
profile.dmc("v",.1,2,p.vector,data.model,ylim=ylim)
profile.dmc("z",.2,.8,p.vector,data.model,ylim=ylim)
profile.dmc("sv",.1,2,p.vector,data.model,ylim=ylim)
profile.dmc("sz",.1,.9,p.vector,data.model,ylim=ylim)
profile.dmc("t0",.01,.5,p.vector,data.model,ylim=ylim)

# Note that z and sz interact when z gets near a boundary and sz is larger. 
# This is because the parameterization assumes that start-point noise is 
# symmetric around z (i.e, z-sz/2 to z+sz/2) but the ends of the range must 
# be within (0,a). 

# If you look at ddm.R you will see a function "bad" in the 
# likelihood.dmc function. It enforces these bounds by 
...
# can occur during sampling. Note that the set implemented in "bad" may need 
# augmentation. 


# Here we use beta(1,1) (i.e., uniform) priors on the z, sz and t0 parameters
# (so they are bounded in the default 0-1 range), and truncated normal priors
# on the remaining parameters (with broad bounds again in order to avoid 
# numerical issues). These choices might vary in other applications, and 
# interact with the likelihood restrictions described above. 
p.prior <- prior.p.dmc(
...
plot.dmc(samples2.auto,p.prior=p.prior,show.obs=FALSE)
