...
system.time({
  samples1 <- run.dmc(samples1, report = 10, cores=4,p.migrate=.05)
})
...
samplesCP <- run.dmc(samplesCP,p.migrate=.05,cores=9)
plot.dmc(samplesCP)
# Seems to have increased autocorrelation quite a lot, so thin
samplesCP <- run.dmc(samples.dmc(samples=samplesCP,nmc=200,thin=10),cores=9)
plot.dmc(samplesCP)
