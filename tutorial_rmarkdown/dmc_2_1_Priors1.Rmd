...
# observations shown as tick marks on the x-axis.
plot.dmc(samples.big3,layout=c(2,3),p.prior=p.prior3,
  prior.lty=2,prior.col="green",prior.lwd=2,
  post.lty=3,post.col="red",post.lwd=3,show.obs=FALSE)
...
# First compare results obtained with p.prior3 (samples.big3) and p.prior4 
# (samples.big4) for data.small
...
# Now compare samples.big3 and samples.small3 (same prior, but different n)
# Less data = more uncertainty:
par(mfcol=c(2,3))
for(i in names(p.vector)){
