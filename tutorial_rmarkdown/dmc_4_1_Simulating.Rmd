...
data.model <- h.simulate.dmc(model,p.prior=p.prior,ns=2,n=1)
# The sampled parameters are
attr(data.model,"parameters")


# Now lets try this with a larger set of subjects
data.model <- h.simulate.dmc(model,p.prior=p.prior,ns=100,n=100)

# The parameter distributions look like the priors
par(mfrow=c(2,4))
for (i in dimnames(attr(data.model,"parameters"))[[2]])
  plot(density(attr(data.model,"parameters")[,i]),xlab=i,main="")

# As do their means
round(apply(attr(data.model,"parameters"),2,mean),2)

# and standard deviations
round(apply(attr(data.model,"parameters"),2,sd),3)
