  ...
  # from the save.simulation output of post.predict
{
  ...
  get.ps <- function (sim, factors,R,include.na=FALSE) 
  {
    ...
    if (include.na) n/np else nok/np
  }  
  ...
  out <- list(pp.df, pp.df.na, RT.df,get.stat(mean,data,sim,tapplyvec),
    get.stat(sd,data,sim,tapplyvec))
  names(out) <- c("pps","pps.NR","RTs","MeanRTs","SDRTs")
  out
}
